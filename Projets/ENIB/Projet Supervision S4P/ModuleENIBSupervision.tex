...
Ce document a pour but de vous initier à l'utilisation de PyQt5 à travers la réalisation d'un programme simple utilisant PyQt5 et les outils développés dans le module \eMod{ENIBSupervision}.\newline
Ne seront pas abordés ici :
\begin{enumerate}
\item L'utilisation de QtDesigner
\item Le descriptif et le détail du code réalisé dans le module \eMod{ENIBSupervision}
\end{enumerate}

\smallSkip
La réalisation de ce projet a amené à la création d'un module \eMod{ENIBSupervision} fournissant des outils facilitant la mise en oeuvre de logiciels de supervision en Python.\newline
...
\item \label{tag-presentation-PLCVar} \eType{PLCVar} \newline
Il s'agit du type principal de ce module, il représente une variable automate et possède les attributs nécessaires comme un \textbf{nom}, une \textbf{adresse}, une \textbf{description} etc... Ce type a été conçu pour fonctionner avec le type suivant : \eType{ModbusPLC}.\newline
Pour plus d'informations sur ce type, se référer à la documentation fournie en annexe.\newline
D'un point de vue organisation, le type \eType{PLCVar} est défini dans le sous-module \eMod{PLC} puis dans le sous-module \eMod{Variable}. \newline
...
D'un point de vue organisation, le type \eType{EventsLoop} est défini dans le sous-module \eMod{EventsLoop}.\newline
Pour plus d'informations sur ce type, se référer à la documentation fournie en annexe.\newline
Pour importer ce type, il faudra par exemple utiliser :
\begin{lstlisting}[language=Python]
from ENIBSupervision.EventsLoop import EventsLoop
\end{lstlisting}
\textcolor{red}{Il est important de noter que ce type n'est normalement pas à utiliser directement}. Il faut préférer l'utilisation du type \eType{ModbusPLC} qui gérera cette boucle événementielle automatiquement.

\item \label{tag-presentation-ModbusPLC} \eType{ModbusPLC} \newline
Il s'agit du second type principal de ce module, il représente un automate programmable industriel et possède les attributs nécessaires comme un \textbf{nom}, une \textbf{adresse IP}, une \textbf{description} etc... Ce type a été conçu pour fonctionner avec le type suivant : \eType{PLCVar}.\newline
Pour plus d'informations sur ce type, se référer à la documentation fournie en annexe.\newline
D'un point de vue organisation, le type \eType{ModbusPLC} est défini dans le sous-module \eMod{PLC} puis dans le sous-module \eMod{ModbusPLC}. \newline
Pour importer ce type, il faudra par exemple utiliser :
%"coloration" du code
\begin{lstlisting}[language=Python]
...
\item \eType{AnimationTables}\newline
Ce type offre la possibilité de faire très facilement des tables d'animations à partir d'une liste de variables de type \eType{PLCVar}. Pour un bon fonctionnement des tables générées, il \textcolor{red}{est nécessaire} d'utiliser le type \eType{EventsLoop} ou \eType{ModbusPLC}.\newline
La fenêtre générée propose des fonctionnalités de forçage, d'actualisation automatique des valeurs lorsque couplé à une \eType{EventsLoop} ou un \eType{ModbusPMC}, d'affichage des tables d'entrées/sorties redimensionnables et la possibilité de la "docker" sur une fenêtre principale de Qt \href{https://doc.qt.io/qt-5/qmainwindow.html}{QMainWindow}.\newline
D'un point de vue organisation, le type \eType{AnimationTables} est défini dans le sous-module \eMod{UI\_class} puis dans le sous-module \newline \eMod{UI\_AnimationTables}.\newline
Pour plus d'informations sur ce type, se référer à la documentation fournie en annexe.\newline
Pour importer ce type, il faudra par exemple utiliser :
\begin{lstlisting}[language=Python]
...
D'un point de vue organisation, le type \eType{DynamicObject} est défini dans le sous-module \eMod{UI\_class} puis dans le sous-module \newline \eMod{UI\_DynamicObject}.\newline
Pour plus d'informations sur ce type, se référer à la documentation fournie en annexe.\newline
Pour importer ce type, il faudra par exemple utiliser :
\begin{lstlisting}[language=Python]
...
\iImage{\rootIm enibSup_DynamicObject_moteur.png}{Exemple de résultat avec un moteur}{0.4}
\iImage{\rootIm enibSup_DynamicObject_verin.png}{Exemple de résultat avec un vérin}{0.4}

\newpage

\item \eType{IndicLight}\newline
Ce type offre la possibilité de représenter très facilement des voyants avec un nombre d'états infinis.\newline
D'un point de vue organisation, le type \eType{IndicLight} est défini dans le sous-module \eMod{UI\_class} puis dans le sous-module \newline \eMod{UI\_IndicLight}.\newline
Pour plus d'informations sur ce type, se référer à la documentation fournie en annexe.\newline
Pour importer ce type, il faudra par exemple utiliser :
\begin{lstlisting}[language=Python]
...
\iImage{\rootIm enibSup_IndicLight.png}{Exemple de résultat avec deux voyants dans deux états différents}{0.4}
...
Pour notre exemple, je vais nommer le dossier "\ePath{SupEx}".\newline

\subsection{Contenu essentiel de la racine du projet}

Nous pouvons ensuite copier le dossier \ePath{ENIBSupervision} fourni dans la racine du projet.\newline
...
\item \ePath{UI\_formFiles} : Ce dossier contiendra les fichiers d'interfaces graphiques créés avec QtDesigner (fichiers \textbf{*.ui})\newline

\item \ePath{UI\_pyFiles} : Ce dossier contiendra les fichiers Python générés depuis les fichiers \textbf{*.ui} du dossier \ePath{UI\_formFiles} à l'aide de l'outil \textbf{pyuic}.\newline
C'est d'ailleurs ici que rentre en jeu un petit script que vous pouvez trouver dans le dossier du module \eMod{ENIBSupervision}. Ce script python se nomme "\ePath{scriptExport.py}". Son fonctionnement est très simple, il fournit une fonction permettant de convertir automatiquement tous les fichiers \ePath{*.ui} d'un dossier en fichier \ePath{*.py} dans autre dossier. Consultez la documentation de ce module pour plus d'informations.\newline
Si vous lancez le script Python, il cherchera par défaut à appliquer ces traitements sur les dossiers mentionnés soit \ePath{UI\_formFiles} et \ePath{UI\_pyFiles}.

\item \ePath{UI\_class} : Ce nom n'est ici d'aucune importance, vous pouvez le modifier comme bon vous semble sans avoir quoique ce soit d'autre à modifier.\newline
...
Ainsi, nous pouvons créer un fichier "\ePath{main.py}" avec son contenu essentiel dans la racine du projet :
...
dans le dossier adapté, soit \ePath{UI\_formFiles} dans la racine de votre projet. Pour cet exemple, je la nommerai \ePath{UI\_MainWindow.ui}\newline
...
Nous allons nous rendre dans le dossier \ePath{UI\_class} dans la racine du projet pour créer un nouveau fichier qui correspondra au module contenant la définition de notre fenêtre principale. (Le coeur du programme)\newline
Pour ma part, ce module portera le même nom que son fichier \ePath{*.ui} associé soit \ePath{UI\_MainWindow.py}.

\iImage{\rootIm enibSup_window_explorer_UIClass_basic.png}{Contenu du dossier \ePath{UI\_class} en l'état}{0.4}
...
class MainWindow(QtWidgets.QMainWindow):
    #code
\end{pyCode}
Où \eType{MainWindow} correspond au type représentant notre fenêtre principale
\footnote
{
Vous pouvez modifier ce nom comme bon vous semble mais que celui-ci a l'avantage indéniable d'être explicite.
}
et la portion de code \eType{(QtWidgets.QMainWindow)} permet d'indiquer à Python que notre type \eType{MainWindow} \italic{hérite} des propriétés du type \eType{QtWidgets.QMainWindow}.\newline
Concrètement, notre type pourra faire tout ce que faisait une \eType{QMainWindow} du module \eType{QtWidgets} plus ce que nous lui apprendrons.
...
pour notre classe. Celui-ci sera appelé lors de l'initialisation d'un nouvel objet de notre type \eType{MainWindow}.\newline
...
Ce paramètre \textcolor{red}{\bold{doit}} figurer dans tous les prototypes des fonctions membres d'une classe.\smallSkip

Revenons-en au constructeur de notre classe \eType{MainWindow}, il existe avec Qt un système de widget parent/enfant trop complexe à expliquer ici. Le principal est de savoir que ce système existe et que avec Qt, la plupart des widgets\footnote{À noter qu'une \eType{QMainWindow} est aussi un widget} instanciés doivent avoir un parent (pour des raisons trop complexes à aborder ici encore une fois). Ce qui implique que tous les widgets aient dans leurs constructeurs un paramètre représentant un potentiel widget parent.\vskip 0cm %Nouveau paragraphe sans saut de ligne
...
from UI_pyFiles.MainWindow import Ui_MainWindow
\end{pyCode}
La première étape est de d'initialiser une variable membre "interface graphique" dont le type portera le nom du fichier \eType{*.ui} que vous avez créé.\newline
...
\end{pyCode}
Explications : 
Pour accéder à des variables membres de la classe, il est nécessaire en Python de \textcolor{red}{systématiquement} utiliser "\textcolor{blue}{\ib{self.}}"
...
Nous allons délaisser notre fichier \ePath{UI\_MainWindow.py} au profit du fichier \ePath{main.py}. En effet, si la définition de la fenêtre s'est faite dans un fichier, l'initialisation et l'affichage de notre fenêtre \underline{principale} se fera depuis le fichier \ePath{main.py}.\smallSkip

Pour le moment, le contenu du fichier \ePath{main.py} doit ressembler à ceci :
\begin{pyCode}
# -*- coding: utf-8 -*-
#on  explicite l’encodage  du  fichier
...
\end{pyCode}
Très succinctement, nous déclarons ici une \eType{QApplication} en lui passant en paramètre les arguments fournis au programme
...
Pour cela, il nous suffit de déclarer une variable de notre type \eType{MainWindow} puis de l'afficher.\newline
...
\end{pyCode}
Et pour la voir apparaître, il suffit d'appeler une fonction : "\eFunc{show()}" sur cette variable, ce qui donne :
\begin{pyCode}
mainWindow.show()
\end{pyCode}
\smallSkip
...
Les outils développés dans le module \bold{enibSupervision} ont été conçus pour fonctionner avec une liste de variables de type \eType{PLCVar}.\smallSkip
...
Afin de pouvoir utiliser le type \eType{PLCVar}, vous aurez besoin de l'importer, ceci peut se faire de la manière suivate :
\begin{pyCode}
from ENIBSupervision.PLC.Variable import PLCVar
\end{pyCode}
...
Cette fonction sera un membre de la classe \eType{MainWindow}.\newline
...
La liste de variables ainsi obtenue doit être stockée dans une variable membre de notre classe \ib{MainWindow}\smallSkip
...
Voici le prototype du constructeur du type \eType{PLCVar}\footnote{Pour plus d'informations sur ce type et son constructeur, se référer à la documentation} :
\begin{pyCode}
...
Et "\italic{tmp\_varList[dcyIndex]}" permet d'accéder à l'élément de la liste "\italic{tmp\_varList}" à l'index "\italic{dcyIndex}".\smallSkip
...
\item Utiliser le type \eType{EventsLoop} du module \eMod{ENIBSupervision} qui fera (presque) tout à votre place.
\end{enumerate}
...
\eMod{ModbusPLC} afin de simplifier la gestion de ces dernières. \\
...
Cette boucle d'événement correspond au type \eType{EventsLoop} du module \eMod{ENIBSupervision}. Elle existe mais l'utilisateur n'a normalement pas à intéragir directement avec elle. C'est pour cela que nous n'aborderons pas son fonctionnement exact ici.\\
...
Maintenant que nous avons mis en place les outils essentiels pour le bon fonctionnement de la suite, nous allons aborder les outils plus avancés mis à la disposition des utilisateurs du module \eMod{ENIBSupervision}.\smallSkip

Mais avant ça, voici un petit rappel de ce à quoi votre fichier \\
\ePath{UI\_MainWindow.py} devrait ressembler au complet : %COMPLETE_FILE
\begin{pyCode}
...
Le module \eMod{ENIBSupervision} permet d'associer une fonction à un changement d'état d'une variable
...
\end{pyCode}
\smallSkip
Pour associer cette fonction au changement d'état de la variable dcy, il nous faut maintenant modifier la déclaration de celle-ci comme expliqué plus haut, ce qui se résume à \footnote{Dans la fonction \eFunc{initVarList}} :
\begin{pyCode}
#déclarations des différentes variables
...
\smallSkip
\textcolor{red}{\bold{Tous les signaux créés spécialement pour le module}} \eMod{ENIBSupervision} \textcolor{red}{\bold{commencent par le préfixe "sig\_"}}
\smallSkip

Pour cet exemple, nous allons associer notre nouvelle variable "alarmButton" à une fonction qui affiche son état.\newline
Il faut donc commencer par définir cette fonction (elle est aussi membre de \eType{MainWindow}) :
\begin{pyCode}
...
Cette instruction prendra place juste après sa déclaration et nous utiliserons le signal de \eType{PLCVar} : \eFunc{sig\_valueChanged} :
\begin{pyCode}
#On relie la variable à la méthode "checkAlarm" de MainWindow
varList[-1].sig_valueChanged.connect(self.checkAlarm)
\end{pyCode}
Cette instruction ne fait rien d'autre que relier le signal \eFunc{sig\_valueChanged} du dernier élément de la liste \eVar{varList} à la fonction \eFunc{self.checkAlarm}.\newline
D'où l'importance de la placer juste après la déclaration de la variable "alarmButton", ce qui devrait vous donner au final pour la fonction \eFunc{initVarList} :
...
Nous arrivons maintenant dans les outils graphiques proposés par le module \eMod{ENIBSupervision}.\newline
...
Cependant, nous en avons maintenant l'habitude, il nous faut tout d'abord importer le type (\eType{AnimationTables}) nécessaire depuis le module adapté, le module \eMod{ENIBSupervision.UI\_class.UI\_AnimationTables}, cela se traduit par :
\begin{pyCode}
from ENIBSupervision.UI_class.UI_AnimationTables import AnimationTables
\end{pyCode}
\smallSkip

Voici le prototype du type \eType{AnimationTables} :
\begin{pyCode}
...
Si l'on analyse rapidement cette déclaration, on constate que nous ne faisons que initialiser une variable \eVar{self.\_\_animTables} avec comme liste de variables \\ \eFunc{self.\_\_plcClientList[self.\_\_plcSelected].varList()} [en choisissant l'index de l'automate] et comme parent, notre \eType{MainWindow}.\newline
...
Nous avons déclaré nos tables, nous n'avons plus qu'à les afficher, ce qui se fera à l'aide de la fonction membre \eFunc{.show()} (toujours à la fin du constructeur).
...
\begin{enumerate}
    ...
    \item On crée une fonction membre appelée \eFunc{showTables} qui, à chaque appel, se contentera d'ouvrir et d'afficher les tables d'animations de l'automate.
    ...
    \end{pyCode}
    \item On relie le signal du bouton à la fonction membre \eFunc{showTables()} \bold{dans le constructeur}' \footnote{En cas d'oubli de méthode, le rappel est au chapitre 8.6}
...
Pour cela nous allons créer une nouvelle fonction qu'il nous faudra \bold{connecter à l'appui sur le bouton "start"} : \eFunc{incrementNewCycleCount} qui exécutera l'action que nous avons expliqués plus haut.\smallSkip
