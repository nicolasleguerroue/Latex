...
Maintenant, ou souhaite définir le comportement de notre objet '\eType{Véhicule}'. Nous allons décréter que notre objet peut 'avancer', 'reculer', 'tourner à droite', etc.
...
\iImage{images/QtDesigner/designer_poo_heritage.png}{L'héritage entre l'objet 'Véhicule' et l'objet 'Voiture'}{0.5}
...
\end{lstlisting}
L'instruction \bold{super.\_\_init\_\_(parent)} est un appel du constructeur parent. \newline
...
Ce signal devra donc déclencher la fonction membre (\eFunc{slot}) 'test\_pb' que nous allons coder à la suite du constructeur. \newline
